...
__declspec(dllexport) LPXLOPER12 XLLdateDigit2Excel(LPXLOPER12 n)
{
	...
	if (error == -1) {
		return XLOPERNumDLLFree(digitdate > 1000000 ? toExcelDate(digitdate) : digitdate);
	}
	else
	{
...
__declspec(dllexport) LPXLOPER12 XLLdateExcel2Digit(LPXLOPER12 n)
{
	...
	if (error == -1) {
		return XLOPERNumDLLFree(serialdate < 1000000 ? fromExcelDate(serialdate) : serialdate);
	}
	else
	{
		return XLOPERErrDLLFree(error);
